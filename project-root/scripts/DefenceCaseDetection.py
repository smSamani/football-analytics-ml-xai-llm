...
def run_full_pipeline(event_json_path, match_name):
    ...
    try:
        ...
        # Identify opponent team id for this match
        OPPONENT_ID = None
        for _e in events:
            tid = _e.get("team", {}).get("id")
            if tid is not None and tid != TEAM_ID_LEVERKUSEN:
                OPPONENT_ID = tid
                break
        ...
        def find_attack_cases(events):
            ...
            for i, event in enumerate(events):
                # check if this is a shot with xG by the opponent
                if (
                    ...
                    # backtrack to find the last non-opponent possession event
                    start_point_id = None
                    start_index = None
                    for j in range(i - 1, -1, -1):
                        prev_event = events[j]
                        if prev_event.get("possession_team", {}).get("id") != OPPONENT_ID:
        ...
        def involved_players(e):
            # Count opponent players involved in the attacking sequence
            if e.get("team", {}).get("id") != TEAM_ID_LEVERKUSEN:
        ...
        for _, row in df_cleaned.iterrows():
            ...
            if events_in_case:
                ...
                if (
                    ...
                    and last_event.get("team", {}).get("id") != TEAM_ID_LEVERKUSEN
...
# === MAIN LOGIC ===
def main():
    BASE_MATCHES_DIR = DATA_DIR / "raw" / "Leverkusen_Matches"
    FINAL_OUTPUT_PATH = CSV_DIR / "DefenceBaseFeatures.csv"
    all_dfs = []
    matches_processed = 0
    total_attacks = 0
    ...
    if all_dfs:
        ...
        print(f"✅ Total opponent attacks: {len(final_df)}")
...
# === ENTRYPOINT ===
def run():
    main()   # assuming your main logic is inside a function called main()
